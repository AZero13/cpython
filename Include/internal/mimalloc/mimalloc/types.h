...
typedef struct mi_page_s {
  ...
  uint64_t                  qsbr_goal;         // QSBR goal for delayed freeing
  struct llist_node         qsbr_node;         // linked list node for QSBR page queue
  bool                      use_qsbr;          // true if this page uses QSBR for deferred freeing
  #endif
} mi_page_t;
...
struct mi_heap_s {
  ...
  #ifdef Py_GIL_DISABLED
  bool                  page_use_qsbr;                       // if true, new pages will use QSBR for deferred freeing
  #endif
};
